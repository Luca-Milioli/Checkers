...
func _check_move() -> bool:
	var old_cell_key = _make_key(self.old_cell.x, self.old_cell.y)
	if (
		(
			self.available_captures.has(old_cell_key)
			and self.available_captures[old_cell_key].has(self.new_cell)
		)
		or (
			self.available_moves.has(old_cell_key)
			and self.available_moves[old_cell_key].has(self.new_cell)
		)
	):
		return true
	return false
...
func _capture():
	var capture_cell = Vector2i(
		max(self.old_cell.x, self.new_cell.x) - 1, max(self.old_cell.y, self.new_cell.y) - 1
	)
...
func _promote():
	self.board[self.new_cell.x][self.new_cell.y] = (
		WHITE_KING if self.player1.is_playing() else BLACK_KING
	)
	self.new_king.emit(old_cell)
	self.available_captures.clear()  # blocks multiple captures
	self.past_board.clear()
...
func _set_move():
	...
	if abs(new_cell.y - old_cell.y) == 2:  # capture
		_capture()
	if (
		new_cell.x == SIZE - 1 and man_moving == BLACK_MAN
		or new_cell.x == 0 and man_moving == WHITE_MAN
	):
		_promote()
...
func _check_count_rule():
	self.move_count_rule += 1
	if self.move_count_rule == MAX_MOVES * 2 - 1:
		_set_draw()


func _update_available_moves():
	if _new_available_captures():
...
func _new_available_captures():
	...
	for x in range(SIZE):
		for y in range(SIZE):
			if (
				self.board[x][y] != NO_MAN
				and (self.board[x][y] <= WHITE_KING) == self.player1.is_playing()
			):
				var man_captures = _single_man_available_captures(x, y)
				if not man_captures.is_empty():
					self.available_captures[_make_key(x, y)] = man_captures
		...
		for y in range(SIZE):
			if (
				...
				if not man_moves.is_empty():
					self.available_moves[_make_key(x, y)] = man_moves
